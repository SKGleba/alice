...
volatile char* l_debug_core_printf_id[4] = { "[ALICE0] ", "[ALICE1] ", "[ALICE2] ", "[ALICE3] " };
volatile bool l_debug_core_printf_use_id[4] = { true,true,true,true };

bool debug_coreID(bool set, bool enabled, char** id) {
    int cpu_core = get_cpu_id();
    if (set) {
        l_debug_core_printf_use_id[cpu_core] = enabled;
        if (id)
            l_debug_core_printf_id[cpu_core] = (char *)id;
    } else if (id)
        *id = (char *)l_debug_core_printf_id[cpu_core];
    return l_debug_core_printf_use_id[cpu_core];
}
...
void debug_printFormat(char* base, ...) {
    ...
    char* id = NULL;
    if (debug_coreID(false, false, &id))
        print(id);
...
static void printRange32(uint32_t* addr, uint32_t size, bool show_addr) {
    ...
    bool hadCoreId = debug_coreID(false, false, NULL);
    debug_coreID(true, false, NULL);
    ...
    debug_coreID(true, hadCoreId, NULL);
